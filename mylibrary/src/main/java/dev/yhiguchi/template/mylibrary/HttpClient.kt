...
object HttpClient {
  ...
  @OptIn(ExperimentalSerializationApi::class)
  suspend inline fun <reified T, reified R> post(url: String, requestBody: T): HttpResponse<R> = withContext(
    Dispatchers.IO
  ) {
  ...
  fun httpURLConnection(url: String, httpMethod: HttpMethod) = (URL(url).openConnection() as HttpURLConnection).also {
  ...
  @OptIn(ExperimentalSerializationApi::class)
  inline fun <reified T> httpURLConnection(url: String, httpMethod: HttpMethod, requestBody: T) = httpURLConnection(url, httpMethod).also {
    it.doOutput = true
    Json.encodeToStream(requestBody, it.outputStream)
  }

  @OptIn(ExperimentalSerializationApi::class)
  inline fun <reified R> resolve(httpURLConnection: HttpURLConnection): HttpResponse<R> = when (val code = httpURLConnection.responseCode) {
    in 200..299 -> {
      when (R::class) {
